...
export class InMemorySsgSyncService implements SsgSyncService {
  private links: Map<UUID, SsgCourseRef> = new Map();
  private syncStatuses: Map<UUID, SsgSyncStatus> = new Map();
  private conflicts: Map<string, SsgConflict> = new Map();
  private fieldMappings: FieldMapping[] = [];
  private realTimeSyncs: Set<UUID> = new Set();
  private ssgCatalog: Map<string, SsgCourseInfo> = new Map();

  constructor() {
    this.initializeMockData();
  }

  async link(courseId: UUID, ssgId: string): Promise<SsgCourseRef> {
    const ref: SsgCourseRef = {
      ssgId,
      lastSyncAt: new Date(),
      metadata: {
        version: '1.0.0',
  ...
  async sync(courseId: UUID): Promise<SsgSyncResult> {
  ...
  async getStatus(courseId: UUID): Promise<SsgSyncStatus> {
  ...
  async enableRealTimeSync(courseId: UUID, webhookUrl?: string): Promise<void> {
    const link = this.links.get(courseId);
    if (!link) {
      throw new Error('Course is not linked to SSG');
    }
  ...
  async disableRealTimeSync(courseId: UUID): Promise<void> {
  ...
  async searchSsgCatalog(query: SsgSearchQuery): Promise<SsgCourseInfo[]> {
  ...
  async importFromSsg(ssgId: string, mappingConfig?: FieldMapping): Promise<Course> {
  ...
  async exportToSsg(courseId: UUID, publishConfig?: PublishConfig): Promise<SsgPublishResult> {
    const link = this.links.get(courseId);
    if (!link) {
      throw new Error('Course is not linked to SSG');
    }
  ...
  async detectConflicts(courseId: UUID): Promise<SsgConflict[]> {
    const status = this.syncStatuses.get(courseId);
    if (!status) {
      throw new Error('Course is not linked to SSG');
    }
  ...
  async updateFieldMapping(mapping: FieldMapping): Promise<void> {
  ...
  async validateSsgCompatibility(courseId: UUID): Promise<CompatibilityReport> {
  ...
  private initializeMockData(): void {
    ...
    const mockCourses: SsgCourseInfo[] = [
      {
        ssgId: 'ssg-course-1',
  ...
  private async performSync(courseId: UUID, ssgId: string): Promise<void> {
    // Simulate sync delay
  ...
  private async updateSyncStatus(courseId: UUID, status: 'synced' | 'error', result: SsgSyncResult): Promise<void> {
  ...
  private getDefaultFieldMapping(): FieldMapping {
...
export interface SsgSyncResult {
  ...
  error?: string;
  details: {
    metadata: { synced: boolean; conflicts: number };
    content: { synced: boolean; conflicts: number };
    assessments: { synced: boolean; conflicts: number };
    media: { synced: boolean; conflicts: number };
  };
}
